#!/bin/sh
# lifted from here: https://github.com/xssnark/mysql-db-sync
# modified to handle innodb fk constraints
...
INNODBCMDS=" --skip-quick --single-transaction  --no-autocommit "
...
cleandb() {
        ...
        echo 'show tables' | mysql --defaults-file=$CLEAN_DEFAULTS_FILE | while read TABLE; do
                ...
                test ! $DRY_RUN && ( echo "SET FOREIGN_KEY_CHECKS=0; drop table if exists $TABLE" | mysql --defaults-file=$CLEAN_DEFAULTS_FILE)
        done
}

import_pipe() {
        test $VERBOSE && (test ! $DRY_RUN && echo "Performing dump/import process..." || echo "Skipping dump/import process")
        test ! $DRY_RUN && mysqldump --defaults-file=$SRC_CNF $MYVERBOSE $DUMPDB $TableList $INNODBCMDS | mysql --defaults-file=$DST_CNF $MYVERBOSE
        test ! $DRY_RUN && ( echo "SET FOREIGN_KEY_CHECKS=1;" | mysql --defaults-file=$CLEAN_DEFAULTS_FILE)
        PIPE_EXIT=$?
